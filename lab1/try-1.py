...
def read_image():
    image = mpimg.imread('/home/yauheni/source/repos/jubilant-funicular/lab1/discuss.png')
...
def study():
    ...
    while current_error > ERROR_MAX:
        ...
        for block in blocks():
            ...
            x1 = y @ second_layer
            delta = x1 - block
            first_layer -= MAX_ALPHA * np.matmul(np.matmul(block.transpose(), delta), second_layer.transpose())
            second_layer -= MAX_ALPHA * np.matmul(y.transpose(), delta)
        for block in blocks():
            y = block @ first_layer
            x1 = y @ second_layer
            delta = x1 - block
            current_error += delta.sum()
